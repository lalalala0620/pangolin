...
def control_thread(q):
    ...
    while True:
        ...
        if (servo_rate != None) and (servo_rate != [0.0, 0.0]):
                ...
                ServoCmd.forward(servo_angle = [20, 0, 0, -20], sleepTime=0.25, servo_rate=servo_rate)

                ServoCmd.forward(servo_angle = [-38, 0, 0, 38], sleepTime=0.25, servo_rate=servo_rate)

                ServoCmd.forward(servo_angle = [-40, -40, 40, 40], sleepTime=0.25, servo_rate=servo_rate)

                ServoCmd.forward(servo_angle = [0, -20, 20, 0], sleepTime=0.25, servo_rate=servo_rate)

                ServoCmd.forward(servo_angle = [0, 38, -38, 0], sleepTime=0.25, servo_rate=servo_rate)
